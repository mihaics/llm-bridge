...
### Follow-ups surfaced by the final Phase-4b review
- **Dedicated HTTP-level hold-open test for the MCP server.** The real-claude e2e exercises the held-open `tools/call` POST end-to-end, and the hermetic `mcp.rs` test proves `call_tool` parks over an in-memory duplex transport — but no hermetic test drives a real HTTP POST against `McpServer` to prove the stateless `json_response` mode keeps the TCP connection open while the handler parks. Worth adding (a tiny HTTP client POSTing `tools/call`, asserting no response arrives until a result is delivered) so the transport-mode fix is locked against future `rmcp` version drift.
- **Idle-timeout semantics for the tools turn (implemented).** `spawn_streaming_unmetered` uses a per-line idle timeout rather than the whole-turn absolute deadline `spawn_streaming` uses, so the parked gap between collection and continuation is not charged against claude's active budget. If a future change reintroduces a whole-turn deadline here, the parked client think-time would again risk killing a healthy child on resume.
